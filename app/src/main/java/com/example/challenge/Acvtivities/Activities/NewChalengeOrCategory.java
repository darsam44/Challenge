...
public class Admin1 extends AppCompatActivity implements View.OnClickListener {
...
//            startActivity(new Intent(Admin.this , Admin.class ));
    ...
    private void AddChallengeAsAdmin() {
        ...
        if ( WhichCheckBox.isEmpty()){
            Toast.makeText(Admin1.this, "You Must Fill At Least One Category", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        for (int i=0; i<WhichCheckBox.size();i++){
            reference.child(WhichCheckBox.get(i)).child("challenge").child(challenge).setValue(infoChallenge).addOnSuccessListener(new OnSuccessListener<Void>() {
                ...
                public void onSuccess(Void aVoid) {
                    edit_chal.setText("");
                    info.setText("");
                    Toast.makeText(Admin1.this, "Challenge has been added!", Toast.LENGTH_SHORT).show();
    ...
    private void AddCategoryAsAdmin(View v) {
        ...
        categoryQuery.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (temp >0){
                    Toast.makeText(Admin1.this, "Category Already Exist, Please Change", Toast.LENGTH_SHORT).show();
                }
                else{
                    ...
                    reference.child(category).setValue(Cat).addOnSuccessListener(new OnSuccessListener<Void>() {
                        ...
                        public void onSuccess(Void aVoid) {
                            edit_chal.setText("");
                            Toast.makeText(Admin1.this, "Category Successfully Added", Toast.LENGTH_SHORT).show();

                        }
                    }).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(Admin1.this, "Category not Added", Toast.LENGTH_SHORT).show();
